...
class GitHubService {
  ...
  async commentGenerated(testReport, usedCache = false) {
    ...
    const cacheNotice = usedCache 
      ? "\n> 🚀 **Performance**: Test code was retrieved from cache (no regeneration needed)\n"
      : "";
...
<sub>Generated by [PR Test Generator](https://github.com/ka-brian/self-testing-github-action)</sub>`);
  ...
  async getCachedTestCode() {
    try {
      ...
      core.warning(`Failed to get cached test code: ${error.message}`);
      return null;
    }
  }

  async cacheTestCode(testCode, changesHash, testPlan) {
    try {
      ...
      const comment = `## 🗄️ Test Code Cache

*This comment contains cached test code for PR #${this.prNumber}*

<!-- TEST_CODE_CACHE_START -->
${encodedCache}
<!-- TEST_CODE_CACHE_END -->

> **Note**: This cache is automatically managed by the PR Test Generator and will be used to speed up subsequent test runs if the PR changes remain similar.
> 
> **Security**: Sensitive credentials have been replaced with placeholders to prevent exposure in PR comments. They will be automatically restored during test execution.

---
<sub>Generated by [PR Test Generator](https://github.com/ka-brian/self-testing-github-action)</sub>`;

      await this.createComment(comment);
      core.info("✅ Test code cached successfully");
    } catch (error) {
      core.warning(`Failed to cache test code: ${error.message}`);
    }
  }

  calculateChangesHash(prContext) {
  ...
  replaceSensitiveCredentials(testCode) {
    ...
    if (sanitizedCode !== testCode) {
      core.info("🔒 Sensitive credentials replaced with placeholders in cached test code");
    }

    return sanitizedCode;
  }

  restoreSensitiveCredentials(testCode) {
    ...
    if (restoredCode !== testCode) {
      core.info("🔓 Restored sensitive credentials from placeholders");
    }
